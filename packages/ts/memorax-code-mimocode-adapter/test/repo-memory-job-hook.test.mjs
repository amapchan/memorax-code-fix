...
test("mimocode repo memory launcher uses the local server runner", () => {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "mimocode-repo-memory-job-")));
  ...
  try {
    ...
    assert.equal(payload.runner, "mimocode");
