...
import {
  ...
  runMiMoCodeRepoMemory,
} from "../src/repo-memory-server-runner.mjs";

test("mimocode repo memory runner creates, prompts, and deletes a background session", async () => {
  const root = await mkdtemp(join(tmpdir(), "mimocode-repo-memory-runner-"));
  const requests = [];
  ...
  try {
    const result = await runMiMoCodeRepoMemory({
      ...
      env: {
        ...
        MEMORAX_CODE_mimocode_SERVER_URL: server.url,
        mimocode_SERVER_USERNAME: "memorax-user",
...
test("mimocode repo memory runner preserves prompt failures and still deletes the session", async () => {
  const root = await mkdtemp(join(tmpdir(), "mimocode-repo-memory-failure-"));
  let deleted = false;
  const server = await startServer(async (request, response) => {
    if (request.method === "POST" && request.url.startsWith("/session?")) {
  ...
  try {
    await assert.rejects(
      runMiMoCodeRepoMemory({
        ...
        env: { MEMORAX_CODE_mimocode_COMMAND: "/opt/mimocode" },
        spawnImpl: () => assert.fail("prompt failures must not start a fallback server"),
      }),
      /mimocode blocking prompt failed with HTTP 500: model unavailable/,
    );
...
test("mimocode repo memory runner rejects native errors despite partial text", async () => {
  for (const [name, wrapped] of [["MessageAbortedError", false], ["UnknownError", true]]) {
    ...
    await assert.rejects(runMiMoCodeRepoMemory({
      serverUrl: "http://127.0.0.1:4096",
      repo: join(tmpdir(), "mimocode-native-error"),
      prompt: "Build Repo Memory.",
    }, {
      env: { MEMORAX_CODE_mimocode_COMMAND: "/opt/mimocode" },
    ...
    }), { message: "mimocode blocking prompt returned an assistant error" });
...
test("mimocode repo memory runner owns a temporary server when the inherited server is unreachable", async () => {
  const root = await mkdtemp(join(tmpdir(), "mimocode-repo-memory-fallback-"));
  const requests = [];
  const killSignals = [];
  let spawnCall;
  let authorization;
  ...
  try {
    const result = await runMiMoCodeRepoMemory({
      ...
      env: {
        ...
        MEMORAX_CODE_mimocode_COMMAND: "/opt/mimocode",
        mimocode_DB: "/shared/mimocode.db",
      },
      fetchImpl: async (url, init) => {
      ...
      spawnImpl(command, args, options) {
        ...
        authorization = `Basic ${Buffer.from(
          `${options.env.mimocode_SERVER_USERNAME}:${options.env.mimocode_SERVER_PASSWORD}`,
        ).toString("base64")}`;
        const child = fakeServerChild(killSignals);
        queueMicrotask(() => {
          child.stderr.write(`\u001b[32mmimocode server listening on ${server.url}\u001b[0m\n`);
        });
        return child;
      },
    ...
    assert.equal(spawnCall.command, "/opt/mimocode");
    assert.equal(spawnCall.options.cwd, root);
    assert.equal(spawnCall.options.env.mimocode_DB, ":memory:");
    assert.equal(spawnCall.options.env.mimocode_SERVER_USERNAME, "memorax-code");
    assert.match(spawnCall.options.env.mimocode_SERVER_PASSWORD, /^[A-Za-z0-9_-]{32}$/);
    assert.deepEqual(requests.map((request) => request.method), ["POST", "POST", "DELETE"]);
