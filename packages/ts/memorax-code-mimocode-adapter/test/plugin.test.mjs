...
import { createMemoraxMiMoCodePlugin } from "../src/plugin.mjs";
import { mimocode_REPO_MEMORY_AGENT } from "../src/repo-memory-server-runner.mjs";
...
test("default desktop workspace marks start and writeback projectless while Git and ordinary directories stay scoped", async () => {
  const root = await mkdtemp(join(tmpdir(), "memorax-mimocode-workspace-"));
  try {
...
test("repo-scoped reminder builders require a Backend-authorized worktree", async () => {
  const evaluations = [];
  const plugin = createMemoraxMiMoCodePlugin({
    backendConnection: { url: "http://127.0.0.1:8787" },
    fetchImpl: responseSequence([], [
      { ok: true },
...
test("chat.message starts missing Repo Memory for the Backend-authorized worktree", async () => {
  const root = await mkdtemp(join(tmpdir(), "memorax-code-mimocode-auto-build-"));
  ...
  try {
    await Promise.all([
      ...
      mkdir(join(MiMoCodeConfigDir, "hooks"), { recursive: true }),
    ]);
    await writeFile(join(MiMoCodeConfigDir, "hooks", "repo-memory-job.mjs"), [
      ...
      "  mimoCodeCommand: process.env.MEMORAX_CODE_mimocode_COMMAND,",
      "  serverUrl: process.env.MEMORAX_CODE_mimocode_SERVER_URL,",
      "}));",
      "",
    ].join("\n"));
    const plugin = createPluginWithoutReminders({
    ...
    process.execPath = join(root, "mimocode");
    ...
    assert.deepEqual(JSON.parse(await readFile(jobLog, "utf8")), {
      ...
      mimoCodeCommand: join(root, "mimocode"),
...
test("managed plugin starts the Backend once and bounds prompt waiting", async () => {
  const root = await mkdtemp(join(tmpdir(), "memorax-code-mimocode-backend-start-"));
  const nodePath = process.execPath;
  const memoraxCodeHome = join(root, "memorax-code-home");
  const MiMoCodeConfigDir = join(root, "mimocode-config");
...
test("managed plugin records load and real user message workspace evidence", async () => {
  const root = await mkdtemp(join(tmpdir(), "memorax-code-mimocode-runtime-evidence-"));
  const memoraxCodeHome = join(root, "memorax-code-home");
  ...
  try {
    ...
    await assert.rejects(
      readFile(join(memoraxCodeHome, "adapters", "mimocode", "session-registry.json")),
      /ENOENT/,
    );
...
test("the managed Repo Memory agent is registered and isolated from prompt handling", async () => {
  ...
  assert.deepEqual(config.agent[mimocode_REPO_MEMORY_AGENT], {
    description: "User-configured Repo Memory agent.",
    mode: "subagent",
    hidden: true,
    disable: true,
...
test("mimocode forwards first-prompt and post-compaction reminders once", async () => {
  const memoraxCodeHome = await mkdtemp(join(tmpdir(), "memorax-code-mimocode-reminder-"));
  const requests = [];
  let hooks;
  try {
    const plugin = createMemoraxMiMoCodePlugin({
...
test("shell.env overwrites the mimocode session identity and prepends the managed CLI path", async () => {
  ...
  const output = {
    env: {
      ...
      MEMORAX_CODE_mimocode_SERVER_URL: "http://127.0.0.1:1/",
  ...
  assert.equal(output.env.MEMORAX_CODE_MEMORY_CLI_TRACE_CLIENT, "mimocode");
  ...
  assert.equal(output.env.MEMORAX_CODE_mimocode_COMMAND, process.execPath);
  assert.equal(output.env.MEMORAX_CODE_mimocode_SERVER_URL, "http://127.0.0.1:4096/");
  assert.equal(output.env.PATH, [cliBinDir, "/usr/bin", "/bin"].join(delimiter));
});

test("shell.env clears inherited session identity without an mimocode session", async () => {
  const plugin = createMemoraxMiMoCodePlugin();
...
test("a loaded plugin follows the managed enabled state without an mimocode restart", async () => {
  const root = await mkdtemp(join(tmpdir(), "memorax-code-mimocode-plugin-state-"));
  const statePath = join(root, "state.json");
  const requests = [];
  try {
...
test("idle follows an mimocode compaction continuation back to the original pending turn", async () => {
  const requests = [];
...
function createPluginWithoutReminders(options) {
  return createMemoraxMiMoCodePlugin({
