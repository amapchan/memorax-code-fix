...
import { ismimoCodeDefaultWorkspace } from "../../memorax-code-adapter-common/src/default-workspace.mjs";
...
import { mimocode_REPO_MEMORY_AGENT } from "./repo-memory-server-runner.mjs";
...
export function createMemoraxMiMoCodePlugin(options = {}) {
  return async ({ client, project, directory, worktree, serverUrl }) => {
    ...
    const workspaceKind = project?.vcs === "git" ? "project"
      : ismimoCodeDefaultWorkspace(workspaceRoot, options.workspaceOptions) ? "projectless" : "local";
    const mimoCodeServerUrl = urlString(serverUrl);
    ...
    recordMiMoCodeWorkspaceEvidence(options, workspaceRoot, "plugin.load");
    ...
    function ensureBackendReady() {
      ...
      if (!backendEnsurePromise) {
        ...
        backendEnsurePromise = ensureBackendAvailable(ensureOptions)
          .catch((error) => {
            debug(options, "mimocode backend recovery failed", error);
          })
          .then(() => {
    ...
    async function flushSession(sessionId, target) {
      ...
      for (const turn of turns) {
        ...
        try {
          ...
          debug(options, "mimocode oversized writeback discarded", error);
          continue;
        }
    ...
    return {
      config: async (config) => {
        if (!pluginEnabled(options)) return;
        config.agent ??= {};
        const configuredAgent = config.agent[mimocode_REPO_MEMORY_AGENT];
        config.agent[mimocode_REPO_MEMORY_AGENT] = {
      ...
      "chat.message": async (input, output) => {
        if (!pluginEnabled(options)) return;
        if (stringValue(input?.agent) === mimocode_REPO_MEMORY_AGENT) return;
        ...
        recordMiMoCodeWorkspaceEvidence(options, workspaceRoot, "chat.message", sessionId);
        const reminderInput = {
        ...
        try {
          ...
          const repoMemoryEnv = MiMoCodeRepoMemoryEnv(options, mimoCodeServerUrl, sessionId);
          if (repoMemoryEnv) {
            scheduleMissingRepoMemoryBuild(repositoryWorktree, {
              debugEnv: "MEMORAX_CODE_mimocode_PLUGIN_DEBUG",
              pluginRoot: options.MiMoCodeConfigDir,
              env: repoMemoryEnv,
          ...
          debug(options, "mimocode turn start failed", error);
        }
        if (!pluginEnabled(options)) return;
        const reminderResult = await evaluateReminder(
          reminderEvaluator,
        ...
        if (turnStartAccepted && reminderResult?.reminder) {
          track(
            ...
            "mimocode reminder trace failed",
          );
        }
      ...
      "shell.env": async (input, output) => {
        if (!pluginEnabled(options)) return;
        output.env.MEMORAX_CODE_MEMORY_CLI_TRACE_CLIENT = "mimocode";
        if (mimoCodeServerUrl) {
          output.env.MEMORAX_CODE_mimocode_SERVER_URL = mimoCodeServerUrl;
          output.env.MEMORAX_CODE_mimocode_COMMAND = stringValue(options.MiMoCodeCommand)
            ?? process.execPath;
        }
        const sessionId = stringValue(input?.sessionID);
      ...
      event({ event }) {
        ...
        if (event?.type === "message.updated") {
          ...
          if (interrupted) {
            track(
              ...
              "mimocode interrupted turn finalization failed",
            );
          }
          return;
        }
        if (event?.type === "session.status" && event.properties?.status?.type === "idle") {
          ...
          if (sessionId) {
            track(queueSessionFlush(sessionId), "mimocode writeback failed");
          }
        }
      },
      async dispose() {
...
export const MemoraxMiMoCodePlugin = createMemoraxMiMoCodePlugin();
export default MemoraxMiMoCodePlugin;
...
async function evaluateReminder(evaluator, reminderOptions, input, options) {
  try {
    ...
    debug(options, "mimocode memory reminder failed", error);
    return undefined;
  }
}
...
async function showUserNotice(client, directory, notice, options) {
  ...
  try {
    ...
    debug(options, "mimocode quota reminder failed", error);
  }
}
...
function debug(options, message, error) {
  if (options.debug !== true && process.env.MEMORAX_CODE_mimocode_PLUGIN_DEBUG !== "1") return;
...
function MiMoCodeRepoMemoryEnv(options, serverUrl, sessionId) {
  ...
  return {
    ...
    MEMORAX_CODE_mimocode_COMMAND: stringValue(options.MiMoCodeCommand) ?? process.execPath,
    MEMORAX_CODE_mimocode_SERVER_URL: serverUrl,
  };
}
...
function backendEnsureOptions(options) {
  const memoraxCodeHome = stringValue(options.memoraxCodeHome);
  const MiMoCodeConfigDir = stringValue(options.MiMoCodeConfigDir);
  const memoraxCodeCommand = stringValue(options.memoraxCodeCommand);
  if (!memoraxCodeHome || !MiMoCodeConfigDir || !memoraxCodeCommand) return undefined;
  return {
    ...
    memoraxCodeCommand,
    nodePath: options.nodePath,
    resolveHomes: () => ({ memoraxCodeHome, MiMoCodeConfigDir }),
    buildStartArgs: (homes, recoveryArguments) => [
      "start",
      "--home", homes.memoraxCodeHome,
      "--mimocode-config-dir", homes.MiMoCodeConfigDir,
      ...recoveryArguments,
    ],
    debug: (message) => debug(options, "mimocode backend recovery skipped", message),
  };
}

function recordMiMoCodeWorkspaceEvidence(options, cwd, event, sessionId) {
  ...
  try {
    ...
    debug(options, "mimocode workspace evidence failed", error);
  }
}
