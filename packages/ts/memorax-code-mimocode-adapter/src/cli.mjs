...
import { readMiMoCodeWorkspaceStatus } from "./diagnostics.mjs";
import { readMiMoCodePluginStatus } from "./plugin-install.mjs";
...
try {
  ...
  if (parsed.command === "status") {
    print(MiMoCodeStatus(options), options);
  } else if (parsed.command === "doctor") {
    const [status, workspace, backend] = await Promise.all([
      MiMoCodeStatus(options),
      readMiMoCodeWorkspaceStatus(options),
...
function printHelp() {
  console.log([
    "Usage: memorax-code-mimocode [status|doctor] [options]",
    "",
    "Lifecycle note: use `memorax-code start --clients mimocode` to install or reconcile the mimocode plugin.",
    "",
    "Options:",
    "  --mimocode-config-dir DIR  mimocode config root (default: MIMOCODE_CONFIG_DIR, XDG_CONFIG_HOME/mimocode, or ~/.config/mimocode)",
...
function parseOptions(args) {
  ...
  return {
    MiMoCodeConfigDir: values["--mimocode-config-dir"] ?? defaultMiMoCodeConfigDir(),
    memoraxCodeHome,
...
function MiMoCodeStatus(options) {
  const status = readMiMoCodePluginStatus(options);
  const MiMoCodeConfigDir = typeof status.state?.MiMoCodeConfigDir === "string"
    ? status.state.MiMoCodeConfigDir
    : options.MiMoCodeConfigDir;
  const ready = status.ok === true
    && status.installed === true
    && status.enabled === true
    && status.current === true
    && status.backendUrlMatches !== false
    && status.MiMoCodeSkills?.ok === true;
  return {
    ...status,
    ok: ready,
    action: "status",
    MiMoCodeConfigDir,
    ...(MiMoCodeConfigDir !== options.MiMoCodeConfigDir
      ? { requestedMiMoCodeConfigDir: options.MiMoCodeConfigDir }
      : {}),
...
function printStatus(result, includeHints = true) {
  ...
  console.log(`mimocode config: ${result.MiMoCodeConfigDir}`);
  if (result.requestedMiMoCodeConfigDir) {
    console.log(`requested mimocode config: ${result.requestedMiMoCodeConfigDir}`);
  }
  console.log(`memorax-code home: ${result.memoraxCodeHome}`);
  console.log(`state: ${result.statePath}`);
  console.log("integration: plugin");
  console.log("provider config: unchanged (mimocode-owned)");
...
function statusHints(result) {
  ...
  if (result.installed !== true || result.enabled !== true || result.current !== true) {
    hints.push("Install or reconcile the managed integration with `memorax-code start --clients mimocode`, then restart or refresh mimocode.");
  }
  if (result.backendUrlMatches === false) {
    hints.push("Adapter state points at a different Backend endpoint; rerun `memorax-code start --clients mimocode`.");
  }
  return hints;
}
...
function doctorHints(result) {
  ...
  if (result.workspace?.ok === false) {
    hints.push("mimocode runtime evidence is unreadable or invalid; inspect or remove that state file, then restart or refresh mimocode.");
  } else if (result.workspace?.captured !== true) {
    hints.push("No mimocode plugin runtime has been observed yet; restart or refresh mimocode, then rerun this command.");
  }
